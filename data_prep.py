...
def sax(ts, sax_size):
    """
    ts must be z-normalized
    """
    cuts = cuts_for_asize(sax_size)
    z_norm_ts = _z_norm(ts)
    positions = [_get_pos_between_cuts(i, cuts) for i in z_norm_ts]
    return [string.ascii_lowercase[i] for i in positions]


def _z_norm(ts):
    mean = np.mean(ts)
    sd = np.std(ts)
    return [(i - mean) / sd for i in ts]


def _get_pos_between_cuts(i, col):
    col = list(col)
    col.append(i)
    col.sort()
    pos = col.index(i)
